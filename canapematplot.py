...
class _Turtle:
    ...
    def goto(self, x, y):
        ...
        y = float(y)
        if self.pen_down:
            self.ax.plot([self.x, x], [self.y, y], linewidth=self.linewidth, color=self.pencolor_value)
        if self.is_filling:
...
# Mise à jour : remplissage par défaut blanc pour toutes les parties sauf les coussins.
COLOR_ASSISE       = "#ffffff"  # blanc (assises/banquettes)
COLOR_ACC          = "#ffffff"  # blanc (accoudoirs)
COLOR_DOSSIER      = "#ffffff"  # blanc (dossiers)
COLOR_CUSHION      = "#d8c4a8"  # beige (coussins)
COLOR_CONTOUR      = "black"
...
def draw_legend(t, tr, tx_cm, ty_cm, items=None, pos="top-right"):
    ...
    **Note :** La légende a été désactivée. Cette fonction ne dessine rien et retourne immédiatement.
    """
    # Légende désactivée : ne rien dessiner.
    return
    left = tr.left_px; bottom = tr.bottom_px
    right = left + tx_cm*tr.scale; top = bottom + ty_cm*tr.scale
